...
private fun GuardDownScreen(layer: String, onResolved: () -> Unit) {
    ...
    // Whether the system is still allowed to stop this app in the background.
    // When it is, a guard found switched off is far likelier to have been taken
    // than given up — see BatteryExemption for the measurement behind that.
    ...
    val droppedWhileAsleep = !isDns && !batteryExempt
        ...
        Column(
            ...
            Text(
                when {
                    ...
                    droppedWhileAsleep ->
                        "Something stopped Bastion in the background — Android " +
                            "switches the guard off whenever this app is " +
                            "force-stopped, and the phone putting it to sleep " +
                            "counts. One tap puts it back, and \"Keep Bastion " +
                            "awake\" on the Guard screen stops it recurring."
                    else ->
